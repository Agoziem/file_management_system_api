...
class UserService:
    async def get_users(
        ...
        if role.lower() in ["admin", "customer"]:
    ...
    async def update_user(
        ...
        user_object = await self.get_user_by_id(user.id, session)
        if not user_object:
            raise ValueError("User not found")
        
        # Check if email is being changed
        if "email" in user_data and user_data["email"] != user_object.email:
        ...
        await session.commit()
        await session.refresh(user_object)  # Refresh instance after commit
        return user_object
